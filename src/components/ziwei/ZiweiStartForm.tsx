...
export default function ZiweiStartForm() {
  const router = useRouter();
  const [y, setY] = useState(2000);
  const [m, setM] = useState(1);
  const [d, setD] = useState(1);
  const [shichen, setShichen] = useState<string>(""); // "" 未選、"0"-"11" 時支
  ...
  function submit() {
    ...
    if (shichen === "") errs.shichen = "紫微斗數以時辰定命宮，請選擇出生時辰";
    if (gender === "") errs.gender = "請選擇性別";
    setErrors(errs);
    if (Object.keys(errs).length > 0) return;
    track({ type: "reading_start", feature: "ziwei" });
    setLoading(true);
  }
  ...
  function goResult() {
    const params = {
      ...
      m,
      d: dSafe,
      h: Number(shichen) * 2, // 時支代表小時
      g: gender as "M" | "F",
      p: place,
    };
    router.push(`/ziwei/result?d=${encodeParams(params)}`);
  }

  if (loading) {
    return (
      <RitualLoading
        messages={["安放紫微星入垣…", "十二宮星曜依序就位…", "你的紫微命盤正在展開…"]}
        onDone={goResult}
      />
    );
  }
  ...
  return (
    <div className="card-mystic p-6 sm:p-8">
      <h1 className="text-center font-serif text-2xl font-bold">
        <span className="text-gradient-gold">🌌 排出你的紫微命盤</span>
      </h1>
      ...
      <div className="mt-6 space-y-5">
        ...
        {/* 出生時辰（必填） */}
        <fieldset>
          <legend className="mb-2 text-sm font-bold text-ink-100">出生時辰（必填）</legend>
          <label className="block">
          ...
          <p className="mt-1.5 text-xs leading-relaxed text-ink-500">
            紫微斗數以出生時辰安命宮，缺少時辰無法排盤。可向家人或出生證明查詢；
            若真的不知道時辰，可以先試試不需時辰的{" "}
            <Link href="/bazi" className="text-mystic-300 underline hover:text-gold-300">八字命盤</Link>。
          </p>
        </fieldset>
